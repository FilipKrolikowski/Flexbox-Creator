...
import { generateGridElements } from "../../../../utils/generateGridElements";
...
function GridArea() {
  ...
  const { gridSections, rowTracks, columnTracks } = generateGridElements(container, items);

  return (
    <>
      {gridSections.map((item) => (
        <div key={item.key} className={styles.grid_section} style={item.style}>
          <button type="button" className={styles.add_btn} onClick={() => addItemStyle(item.style)}>
