...
import { ContainerConfig } from "../../../../data/dataTypes";
      ...
      {containerConfig.map((item) => (
        <Fragment key={item.key}>
          {item.itemType === "dropdown" && (
            <ItemDropdown
              item={item}
              value={container[item.key as keyof Container]?.toString() || item.defaultValue}
              separator={item.dropDownSeparator || " "}
              onChange={(key, value) => editContainer(key as keyof Container, value)}
            />
          )}
