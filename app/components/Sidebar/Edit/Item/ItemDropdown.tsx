...
const Item = ({ item, value, separator, onChange }: Props) => {
  ...
  return (
    <>
      <motion.label className={styles.item} key={item.key} layout onClick={toggleOpen}>
      ...
      <AnimatePresence mode="popLayout">
        {open && (
          ...
          >
              ...
              {item.dropDownType === "combine" &&
                item.combineData.map((value, i) => (
                  <motion.label
                    ...
                    layout
                  >
                    <div className={styles.dropdown_title}>{value.title}</div>
                    {item.type === "input" && item.inputType === "unit" && (
                      <TextInput
                        size="small"
                        value={valueItems[i]}
            ...
            {item.dropDownType === "iteration" && (
              <button className={styles.add_btn} onClick={() => addValue("1fr")}>
                <MdAddCircleOutline />
                Add
              </button>
            )}
