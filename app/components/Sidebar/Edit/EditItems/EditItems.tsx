...
import { ItemConfig } from "../../../../data/dataTypes";
...
function EditItems({ selectedItems, selectedItemStyles, editItemStyle, itemsConfig }: Props) {
...
function ItemsConfig({ selectedItemStyles, editItemStyle, itemsConfig }: Props) {
  return (
    <>
      {itemsConfig?.map((item) => (
        <Fragment key={item.key}>
          {item.itemType === "dropdown" && (
            <ItemDropdown
              item={item}
              value={selectedItemStyles?.[item.key as keyof ItemStyle]?.toString() || item.defaultValue}
              separator={item.dropDownSeparator || " "}
              onChange={(key, value) => editItemStyle(key as keyof ItemStyle, value)}
            />
          )}
