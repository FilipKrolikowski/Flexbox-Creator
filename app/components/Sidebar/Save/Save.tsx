...
function Save({ storageKey, state, setState }: Props) {
  ...
  return (
    <div className={styles.container}>
      ...
      <Reorder.Group axis="y" values={edits} onReorder={setEdits} className={styles.list}>
        <AnimatePresence mode="popLayout" initial={false}>
          {edits.map((edit) => (
            <SaveItem key={edit.name} edit={edit} handleView={handleView} handleDelete={handleDelete} />
          ))}
        </AnimatePresence>
      </Reorder.Group>

      {edits.length > 0 && (
        <motion.button layout className={styles.clear__btn} onClick={Clear} type="button">
          Clear All Saves
        </motion.button>
      )}
    </div>
