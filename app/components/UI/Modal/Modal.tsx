...
import { cloneElement, createContext, ReactElement, ReactNode, useContext, useEffect, useState } from "react";
    ...
    <AnimatePresence>
      {showModal && (
        <motion.div
          ...
          exit="exit"
        >
          <motion.div className={styles.modal} variants={modal} layout transition={{ duration: 0.2 }}>
            <button className={styles.close__btn} onClick={closeModal}>
              <IoCloseSharp />
            </button>
            {children}
          </motion.div>
          <motion.div className={styles.overlay} onClick={closeModal}></motion.div>
        </motion.div>
      )}
    </AnimatePresence>,
    document.body
