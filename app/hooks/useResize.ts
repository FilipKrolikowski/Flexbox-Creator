...
interface ReturnProps {
  ...
  isResizing: boolean;
  startResize: (event: React.MouseEvent | MouseEvent, direction: "horizontal" | "vertical") => void;
  reset: (direction: "horizontal" | "vertical") => void;
}

export const useResize = ({ ref, minWidth = 400, minHeight = 280 }: Props): ReturnProps => {
  ...
  const startResize = useCallback(
    (event: React.MouseEvent | MouseEvent, direction: "horizontal" | "vertical") => {
      ...
      const handleMouseMove = (e: MouseEvent) => {
        if (direction === "horizontal") {
          const newWidth = Math.max(
            Math.min(startWidth + (e.clientX - startX), Number(maxDimensions.width) || Infinity),
            minWidth
          );
          setDimensions((prev) => ({ ...prev, width: newWidth }));
        } else {
          const newHeight = Math.max(
            Math.min(startHeight + (e.clientY - startY), Number(maxDimensions.height) || Infinity),
            minHeight
          );
