    ...
    switch (action.type) {
      case undo.type: {
        ...
        return {
          ...
          future: [trackKeys.reduce((acc, key) => ({ ...acc, [key]: present[key] }), {}), ...future],
        };
      }
      ...
      case redo.type: {
        ...
        return {
          past: [...past, trackKeys.reduce((acc, key) => ({ ...acc, [key]: present[key] }), {})],
          present: newPresent as T,
          future: newFuture,
        };
      }
      ...
      default: {
        ...
        const hasChangedTrackedKeys = trackKeys.some((key) => present[key] !== newPresent[key]);
        ...
        const trackedState = trackKeys.reduce((acc, key) => ({ ...acc, [key]: present[key] }), {});
        return {
...
export const canUndo = <T>(state: HistoryEnabledState<T>) => state.past.length > 0;
export const canRedo = <T>(state: HistoryEnabledState<T>) => state.future.length > 0;
